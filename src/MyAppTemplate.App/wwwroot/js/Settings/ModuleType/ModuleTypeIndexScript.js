"use strict";

$(function () {
    var dataTableInstance = null;
    var editModal = new bootstrap.Modal(document.getElementById('modalEditModuleType'));
    var iconPickerModal = new bootstrap.Modal(document.getElementById('modalIconPickerMT'));

    var bootstrapIcons = [
        'bi bi-house', 'bi bi-file-text', 'bi bi-gear', 'bi bi-person', 'bi bi-people',
        'bi bi-lock', 'bi bi-unlock', 'bi bi-check', 'bi bi-x', 'bi bi-plus',
        'bi bi-dash', 'bi bi-trash', 'bi bi-pencil', 'bi bi-eye', 'bi bi-eye-slash',
        'bi bi-search', 'bi bi-funnel', 'bi bi-download', 'bi bi-upload', 'bi bi-link-45deg',
        'bi bi-diagram-3', 'bi bi-boxes', 'bi bi-bookmark', 'bi bi-star', 'bi bi-star-fill',
        'bi bi-heart', 'bi bi-bell', 'bi bi-calendar', 'bi bi-clock', 'bi bi-sliders',
        'bi bi-bar-chart', 'bi bi-pie-chart', 'bi bi-graph-up', 'bi bi-graph-down', 'bi bi-database',
        'bi bi-server', 'bi bi-shield-lock', 'bi bi-key', 'bi bi-check-circle', 'bi bi-folder',
        'bi bi-archive', 'bi bi-clock-history', 'bi bi-info-circle', 'bi bi-question-circle',
        'bi bi-exclamation-circle', 'bi bi-circle-fill', 'bi bi-wrench', 'bi bi-tools', 'bi bi-briefcase',
        'bi bi-credit-card', 'bi bi-wallet', 'bi bi-gift', 'bi bi-flag', 'bi bi-play',
        'bi bi-pause', 'bi bi-stop', 'bi bi-arrow-right', 'bi bi-arrow-left', 'bi bi-arrow-up',
        'bi bi-arrow-down', 'bi bi-arrow-repeat', 'bi bi-arrow-clockwise', 'bi bi-share', 'bi bi-lightning-fill',
        'bi bi-power', 'bi bi-list', 'bi bi-menu-button', 'bi bi-grid', 'bi bi-card-list',
        'bi bi-speedometer', 'bi bi-zoom-in', 'bi bi-zoom-out', 'bi bi-telephone', 'bi bi-chat',
        'bi bi-envelope', 'bi bi-globe', 'bi bi-map', 'bi bi-rss', 'bi bi-award', 'bi bi-ribbon',
        'bi bi-collection'
    ];

    var $perms = $('#pagePermissions');
    var canUpdate = $perms.data('can-update') === true;
    var canDelete = $perms.data('can-delete') === true;

    initializeDataTable();

    $('#btnRefreshModuleTypes').on('click', function () {
        if (dataTableInstance)
            dataTableInstance.ajax.reload(null, false);
    });

    function initializeDataTable() {
        if (dataTableInstance)
            return;

        dataTableInstance = window.initializeDataTable({
            tableSelector: '#tableModuleTypes',
            ajaxUrl: '/ModuleType/Inquire',
            ajaxData: function (d) {
                d.isActive = null;
            },
            searchInputSelector: '#tableModuleTypesSearch',
            columns: [
                { data: 'Name' },
                {
                    data: 'Description',
                    render: function (data) {
                        if (!data) return '<span class="text-muted">—</span>';
                        return `<span class="text-truncate d-inline-block" style="max-width: 200px;" title="${data}">${data}</span>`;
                    }
                },
                {
                    data: 'Icon',
                    class: 'text-center',
                    render: function (data) {
                        if (!data) return '<span class="text-muted">—</span>';
                        return `<i class="${data}" title="${data}"></i>`;
                    }
                },
                {
                    data: 'DisplayOrder',
                    class: 'text-center'
                },
                {
                    data: 'IsActive',
                    class: 'text-center',
                    render: function (data) {
                        return data
                            ? `<i class="bi bi-check-circle-fill text-success" style="font-size:1rem;"></i>`
                            : `<i class="bi bi-x-circle-fill text-danger" style="font-size:1rem;"></i>`;
                    }
                },
                {
                    data: 'InMaintenance',
                    class: 'text-center',
                    render: function (data) {
                        return data
                            ? `<i class="bi bi-cone-striped text-warning" style="font-size:1rem;" title="In Maintenance"></i>`
                            : `<span class="text-muted">—</span>`;
                    }
                },
                {
                    data: 'CreatedByDisplayName',
                    class: 'text-center',
                    render: function (data) { return data ?? 'N/A'; }
                },
                {
                    data: 'CreatedDate',
                    render: function (data) { return window.DataTableRenderers.formatDate(data, "YYYY-MM-DD HH:mm:ss"); }
                },
                {
                    data: 'ModifiedByDisplayName',
                    class: 'text-center',
                    render: function (data) { return data ?? 'N/A'; }
                },
                {
                    data: 'ModifiedDate',
                    render: function (data) { return window.DataTableRenderers.formatDate(data, "YYYY-MM-DD HH:mm:ss"); }
                },
                {
                    data: 'Id',
                    orderable: false,
                    class: 'text-center',
                    render: function (data, type, row) {
                        var editBtn = canUpdate ? `
                            <button class="btn-action btn-action-info edit-btn"
                                data-id="${data}" data-item-name="${row.Name}" title="Edit">
                                <i class="bi bi-pencil"></i>
                            </button>` : '';
                        var deleteBtn = canDelete ? `
                            <button class="btn-action btn-action-danger delete-btn"
                                data-id="${data}" data-item-name="${row.Name}" title="Delete">
                                <i class="bi bi-trash"></i>
                            </button>` : '';
                        return editBtn + deleteBtn || '<span class="text-muted">—</span>';
                    }
                }
            ],
            language: {
                emptyIcon: 'bi bi-collection-x',
                searchIcon: 'bi bi-search',
            },
            order: [[3, 'asc']],
            responsive: true,
            searchHighlight: true
        });
    }

    // ── Icon Picker ───────────────────────────────────────────

    function populateIconPicker() {
        var $grid = $('#iconPickerGridMT');
        $grid.empty();

        bootstrapIcons.forEach(function (iconClass) {
            var $btn = $('<button>')
                .attr('type', 'button')
                .attr('data-icon', iconClass)
                .addClass('btn btn-outline-secondary p-2')
                .html('<i class="' + iconClass + ' fs-5"></i>')
                .on('click', function (e) {
                    e.preventDefault();
                    $('#ModuleType_Icon').val(iconClass);
                    $('#iconPreviewEl').attr('class', iconClass);
                    iconPickerModal.hide();
                });
            $grid.append($btn);
        });
    }

    $('#btnIconPickerMT').on('click', function () {
        populateIconPicker();
        iconPickerModal.show();
        setTimeout(function () {
            var backdrop = document.querySelector('.modal-backdrop:last-of-type');
            if (backdrop) backdrop.style.zIndex = '1060';
        }, 10);
    });

    $('#iconSearchInputMT').on('input', function () {
        var searchTerm = $(this).val().toLowerCase().trim();
        $('#iconPickerGridMT button').each(function () {
            var iconClass = $(this).data('icon');
            $(this).toggleClass('d-none', !iconClass.toLowerCase().includes(searchTerm) && searchTerm !== '');
        });
    });

    // ── Icon input preview update ──────────────────────────────

    $('#ModuleType_Icon').on('input', function () {
        var val = $(this).val().trim();
        $('#iconPreviewEl').attr('class', val || 'bi bi-collection');
    });

    // ── Add ──────────────────────────────────────────────────

    $('#btnAddModuleType').on('click', function () {
        $('#modalEditModuleTypeLabel').text('Add Module Type');
        $('#formEditModuleType')[0].reset();
        $('#ModuleType_Id').val(0);
        $('#ModuleType_IsActive').prop('checked', true);
        $('#ModuleType_InMaintenance').prop('checked', false);
        $('#ModuleType_DisplayOrder').val(0);
        $('#iconPreviewEl').attr('class', 'bi bi-collection');
        editModal.show();
    });

    // ── Edit ─────────────────────────────────────────────────

    $(document).on('click', '.edit-btn', function () {
        var id = $(this).data('id');

        $.get('/ModuleType/GetById', { id: id }, function (mt) {
            $('#modalEditModuleTypeLabel').text('Edit Module Type');
            $('#ModuleType_Id').val(mt.Id);
            $('#ModuleType_Name').val(mt.Name);
            $('#ModuleType_Description').val(mt.Description ?? '');
            $('#ModuleType_Controller').val(mt.Controller ?? '');
            $('#ModuleType_Action').val(mt.Action ?? '');
            $('#ModuleType_Icon').val(mt.Icon ?? '');
            $('#ModuleType_DisplayOrder').val(mt.DisplayOrder);
            $('#ModuleType_IsActive').prop('checked', mt.IsActive);
            $('#ModuleType_InMaintenance').prop('checked', mt.InMaintenance);
            $('#iconPreviewEl').attr('class', mt.Icon || 'bi bi-collection');

            editModal.show();
        }).fail(function () {
            Swal.fire('Error', 'Failed to load module type data.', 'error');
        });
    });

    // ── Save ─────────────────────────────────────────────────

    $('#btnSaveModuleType').on('click', function () {
        var $form = $('#formEditModuleType');
        if (!$form[0].checkValidity()) {
            $form[0].reportValidity();
            return;
        }

        var isNew = parseInt($('#ModuleType_Id').val()) === 0;

        var model = {
            Id:            parseInt($('#ModuleType_Id').val()),
            Name:          $('#ModuleType_Name').val().trim(),
            Description:   $('#ModuleType_Description').val().trim() || null,
            Icon:          $('#ModuleType_Icon').val().trim() || null,
            DisplayOrder:  parseInt($('#ModuleType_DisplayOrder').val()) || 0,
            IsActive:      $('#ModuleType_IsActive').is(':checked'),
            InMaintenance: $('#ModuleType_InMaintenance').is(':checked')
        };

        var $btn = $(this);
        $btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm"></span>');

        $.ajax({
            url: '/ModuleType/Save',
            method: 'POST',
            data: model,
            success: function () {
                editModal.hide();
                dataTableInstance.ajax.reload(null, false);
                Swal.fire({ icon: 'success', title: 'Saved', text: isNew ? 'Module type created successfully.' : 'Module type updated successfully.', timer: 1500, showConfirmButton: false });
            },
            error: function (xhr) {
                Swal.fire('Error', xhr.responseText || 'Failed to save module type.', 'error');
            },
            complete: function () {
                $btn.prop('disabled', false).text('Save Changes');
            }
        });
    });

    // ── Delete ───────────────────────────────────────────────

    $(document).on('click', '.delete-btn', function () {
        var id   = $(this).data('id');
        var name = $(this).data('item-name');

        Swal.fire({
            title: 'Delete Module Type',
            html: `Are you sure you want to delete <strong>${name}</strong>?<br/>This action cannot be undone.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Yes, Delete',
            cancelButtonText: 'Cancel',
            confirmButtonColor: 'var(--bs-danger)',
        }).then(function (result) {
            if (!result.isConfirmed) return;

            $.ajax({
                url: '/ModuleType/Delete',
                method: 'POST',
                data: { id: id },
                success: function () {
                    dataTableInstance.ajax.reload(null, false);
                    Swal.fire({ icon: 'success', title: 'Deleted', text: `${name} has been deleted.`, timer: 1500, showConfirmButton: false });
                },
                error: function (xhr) {
                    Swal.fire('Error', xhr.responseText || 'Failed to delete module type.', 'error');
                }
            });
        });
    });
});
